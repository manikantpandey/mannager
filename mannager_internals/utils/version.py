...
from .dot_mannager import CACHED_VERSIONS_DIR

EXPIRE_PERIOD = 60 * 60 * 4  # 4 hours
TIMEOUT = 5


def get_cached_latest_version(root_path: Path, package_name="mannager"):
...
def update_cached_latest_version(
    root_path: Path, version: Version, package_name="mannager"
):
    cached_file = root_path / CACHED_VERSIONS_DIR / f"{package_name}.json"
