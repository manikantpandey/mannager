...
from mannager_internals.utils.dot_mannager import DOT_mannager_FOLDER_NAME
from mannager_internals.utils.file import (
...
class FilesFromDirectoryTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.path = init_dir()
        self.initial_files = [
            self.path.joinpath("mannager.json"),
    ...
    def test_ignore_file(self):
        self.add_file(".mannagerignore", "ignored")
    ...
    def test_ignore_file_pattern(self):
        self.add_file(".mannagerignore", "~$*.xlsx")
    ...
    def test_ignore_wildcard(self):
        ...
        tracked = self.add_file("tracked", "bar")
        self.add_file(".mannagerignore", "*.ipynb")
        files = files_from_directory(self.path)
        self.assertSamePaths(files, [tracked, *self.initial_files])

    def test_ignore_mannager_folder(self):
        mannager = self.add_folder(DOT_mannager_FOLDER_NAME)
        self.add_file("credentials", "SECRET", path=mannager)
        files = files_from_directory(self.path)
        self.assertSamePaths(files, [*self.initial_files])
    ...
    def test_ignore_mannager_ignore_file(self):
        self.add_file(".mannagerignore", "ignored")
        files = files_from_directory(self.path)
        self.assertSamePaths(files, [*self.initial_files])
    ...
    def test_ignore_mannager_subpath(self):
        self.add_file(".mannagerignore", "ignored")
    ...
    def test_ignore_mannager_files_on_folder(self):
        self.add_file(".mannagerignore", "*/ignored")
    ...
    def test_ignore_mannager_files_on_folder_windows(self):
        self.add_file(".mannagerignore", "*\\ignored")
        self.add_folder("whatever")
        self.add_file("ignored", "foo", self.path.joinpath("whatever"))
        tracked = self.add_file("tracked", "bar", self.path.joinpath("whatever"))
