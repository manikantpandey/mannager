...
from mannager_internals.entities.execution import PreExecution
from mannager_internals.environment import QUEUE_CONCURRENCY, RABBITMQ_EXECUTION_QUEUE
from mannager_internals.logger import mannagerLogger
from mannager_internals.repositories.producer import QueueMessage
from mannager_internals.utils import deserialize
...
class RabbitConsumer(Consumer):
    ...
    def _callback_threadsafe(self, msg: QueueMessage):
        if self.channel.is_open:
            mannagerLogger.debug(f"[CONSUMER] ack {msg.delivery_tag}")
            self.channel.basic_ack(msg.delivery_tag)
        else:
            mannagerLogger.capture_exception(
    ...
    def iter(self) -> Generator[QueueMessage, None, None]:
        mannagerLogger.warning("[CONSUMER] starting")
        while True:
            if self.stop_evt.is_set():
                mannagerLogger.warning("[CONSUMER] exiting iteration")
                break
    ...
    def stop(self):
        mannagerLogger.warning("[CONSUMER] stoping")
        self.stop_evt.set()

    def close(self):
        mannagerLogger.warning("[CONSUMER] channel canceling")
        self.channel.cancel()
        mannagerLogger.warning("[CONSUMER] channel closing")
        self.channel.close()
        mannagerLogger.warning("[CONSUMER] connection closing")
