...
from mannager_internals.interface.cli.deploy import _generate_zip_file
...
def generate_compat_lines(script_type: str):
    return [
        ...
        "https://docs.mannager.io/guides/use-legacy-threads/\n\n",
        "The new way of using workflows is with tasks. Learn more\n",
        "at https://docs.mannager.io/concepts/tasks/ and contact us\n",
...
class Migration012(Migration):
    ...
    def backup_project_on_dot_mannager(self):
        root = Settings.root_path
        dot_mannager_folder = root / DOT_mannager_FOLDER_NAME
        zip_path = str(_generate_zip_file())
        backup_zip_path = dot_mannager_folder / "backup.zip"
        shutil.move(zip_path, backup_zip_path)

    def _migrate(self) -> None:
        if not self.test_mode:
            self.backup_project_on_dot_mannager()
