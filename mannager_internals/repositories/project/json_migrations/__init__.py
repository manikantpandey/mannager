...
from mannager_internals.utils.dot_mannager import CONFIG_FILE_BACKUPS
...
def migrate(data: dict, path: Path, verbose=True):
    ...
    if verbose:
        print("Your mannager.json is outdated, running migrations...")

    try:
        filename = f"mannager_{datetime.now().strftime('%Y%m%d%H%M%S')}.json.backup"
    ...
    if verbose:
        print("Your mannager.json is up to date ✅")
