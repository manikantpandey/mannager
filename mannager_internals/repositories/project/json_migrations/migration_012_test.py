...
from mannager_internals.repositories.project.json_migrations.migration_012 import (
    Migration012,
)
...
class TestMigration12(TestCase):
    def test_base_json(self):
        base_mannager_json = Path(__file__).parent / "json_tests" / "base.json"
        json_data = json.loads(base_mannager_json.read_text())
    ...
    def test_simple_json(self):
        simple_mannager_json = Path(__file__).parent / "json_tests" / "simple.json"
        json_data = json.loads(simple_mannager_json.read_text())
        expected_result = copy.deepcopy(json_data)
        m = Migration012(json_data)
        m.set_as_test()
        m.apply()
