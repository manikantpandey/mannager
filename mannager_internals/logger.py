...
from mannager_internals.environment import LOGFORMAT, LOGLEVEL, NOISY_LOGLEVEL
from mannager_internals.utils.env import is_dev_env, is_test_env

internal_logger = lambda: logging.getLogger("mannager_internal")  # noqa: E731
...
class DevSDK:
    ...
    def capture_exception(cls, exception: BaseException):
        internal_logger().exception(
            msg=f"[mannager_LOGGER] Exception captured: {exception}"
        )

    @classmethod
    def capture_message(cls, message):
        internal_logger().info(f"[mannager_LOGGER] Message captured: {message}")
    ...
    def init(cls, environment: Optional[Environment]):
        ...
        try:
            cls.get_sdk().init(
                ...
                enable_tracing=True,
                release=pkg_resources.get_distribution("mannager").version,
                shutdown_timeout=0,
            ...
            internal_logger().error(
                "[mannager_LOGGER] Error reporting has been turned off."
            )
