...
from mannager.forms import url_params
from mannager.tasks import get_tasks, get_trigger_task, send_task
...
from mannager_internals.utils.json import to_json_serializable
...
def use_legacy_threads(mode: Literal["scripts", "forms", "jobs", "hooks"]):
    ...
    workflow = WorkflowModule("mannager.workflows")
    ...
    if mode == "scripts":
        ...
        if len(pending_tasks) > 0:
            task = next(
                ...
                if t.get_payload().get("_id") == url_params["mannager-run-id"]
            )
            execution.context.legacy_thread_data = task.get_payload()
    ...
    sys.modules["mannager.workflows"] = workflow
